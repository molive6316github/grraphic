...
serve(async (req) => {
  ...
  try {
    ...
    if (grant_type === 'authorization_code') {
      ...
      // Get client and verify credentials
      ...
      // Store access token
      const { data: storedAccessToken, error: accessError } = await supabaseAdmin
      ...
      // Get current scopes from access token
      const { data: accessTokenData } = await supabaseAdmin
        .from('oauth_access_tokens')
        .select('scopes')
      ...
      const newAccessTokenExpiry = new Date(Date.now() + 60 * 60 * 1000); // 1 hour

      const { error: newAccessError } = await supabaseAdmin
        .from('oauth_access_tokens')
        .insert({
      ...
      return new Response(
        JSON.stringify({ error: 'unsupported_grant_type', error_description: 'Only authorization_code and refresh_token grants are supported' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
  } catch (err) {
